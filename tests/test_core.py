...
class TestControllerRun:
    ...
    def test_auto_profile_detects_anycubic_startup(self, tmp_path):
        gcode = "\n".join([
            "; generated by AnycubicSlicerNext",
            "G9111 bedTemp=75 extruderTemp=230",
        ...
        gcode_path = _write_gcode(tmp_path, gcode, name="auto_profile.gcode")
        ctrl = Controller(profiles_dir=str(tmp_path / "profiles"))
        request = ResumeRequest(
            input_path=str(gcode_path),
            resume_selector=1,
        ...
        result = ctrl.run(request)
        assert result.output_path.exists()
        assert any("Auto profile selected: anycubic" in w for w in result.warnings)

    def test_auto_profile_detects_bambu_orca_startup(self, tmp_path):
        gcode = "\n".join([
        ...
        gcode_path = _write_gcode(tmp_path, gcode, name="auto_profile_bambu.gcode")
        ctrl = Controller(profiles_dir=str(tmp_path / "profiles"))
        request = ResumeRequest(
            input_path=str(gcode_path),
            resume_selector=1,
        ...
        result = ctrl.run(request)
        assert result.output_path.exists()
        assert any("Auto profile selected: bambu_orca" in w for w in result.warnings)
